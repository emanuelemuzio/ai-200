---
title: "Automate container image builds and maintenance with Azure Container Registry tasks"
...
- Provide cloud-based container image building for Linux, Windows, and ARM architectures.
...
For example, with triggers for updates to a base image, you can automate OS and framework patching for your Docker containers. These triggers can help you maintain secure environments while adhering to the principles of immutable containers.

> **Important:** Azure Container Registry task runs are temporarily paused from Azure free credits. This pause might affect existing task runs. If you encounter problems, open a support case for the Azure team to provide additional guidance.
> **Warning:** Please be advised that any information provided on the command line or as part of a URI may be logged as part of Azure Container Registry (ACR) diagnostic tracing. This includes sensitive data such as credentials, GitHub personal access tokens, and other secure information. Exercise caution to prevent any potential security risks; it is crucial to avoid including sensitive details in command lines or URIs that are subject to diagnostic logging.

## Task scenarios

Azure Container Registry tasks support several scenarios to build and maintain container images and other artifacts. This article describes quick tasks, automatically triggered tasks, and multi-step tasks.

Each task has an associated source code context, which is the location of source files that are used to build a container image or other artifact. Example contexts include a Git repository and a local file system.
...
By using the familiar `docker build` format, the Azure CLI `az acr build` command takes a context. The command then sends the context to Azure Container Registry and, by default, pushes the built image to its registry upon completion.

Azure Container Registry tasks are designed as a container lifecycle primitive. For example, you can integrate Azure Container Registry tasks into your continuous integration and continuous delivery (CI/CD) solution. If you run `az login` with a service principal, your CI/CD solution can then issue `az acr build` commands to start image builds.

To learn how to use quick tasks, you can follow the typical quick-start and tutorial flow for building and deploying container images by using Azure Container Registry tasks.

Tip

If you want to build and push an image directly from source code, without a Dockerfile, Azure Container Registry provides the `az acr pack build` command (preview). This tool builds and pushes an image from application source code by using Cloud Native Buildpacks.
...
> **Note:** Currently, Azure Container Registry tasks don't support commit or pull-request triggers in GitHub Enterprise repos.

To learn how to trigger builds on source code commits, see the automation patterns for source-driven builds in Azure Container Registry tasks.
...
To create a personal access token, see the GitHub or Azure DevOps documentation.
...
6. Perform a `helm upgrade` task by using the new Helm chart archive package.
...
|Artifact in container registry|OCI artifact files in a container registry repository.|`oci://myregistry.azurecr.io/myartifact:mytag`|

> **Note:** When you're using a Git repo as a context for a task that's triggered by a source code update, you need to provide a personal access token.
...
Each task run generates log output that you can inspect to determine whether the task steps ran successfully. When you trigger a task manually, log output for the task run is streamed to the console and stored for later retrieval. When a task is triggered automatically (for example, by a source code commit or a base image update), task logs are only stored. View the run logs in the Azure portal, or use the Azure CLI task logs command.

Learn more about viewing and managing task logs.

## Concepts

- Dependency tracking: ACR tasks can react to base image updates and rebuild dependent images without requiring manual follow-up.
- Image lifecycle: build, push, validation, and deployment are part of one continuous pipeline that keeps image delivery consistent. See [Additional Resources/Build and deploy container images with ACR Tasks.md](Build%20and%20deploy%20container%20images%20with%20ACR%20Tasks.md).
- Pipeline orchestration: multi-step workflows let teams validate artifacts before publication and keep release logic explicit. See [Additional Resources/Run multi-step build, test, and patch tasks.md](Run%20multi-step%20build,%20test,%20and%20patch%20tasks.md).
- Governance: access control, auditing, and naming discipline become essential as automated builds move into shared registry workflows.
