---
title: About registries, repositories, and artifacts
...
Repository names can also include namespaces. By using forward slash-delimited names, you can identify related repositories and artifact ownership in your organization. However, the registry manages all repositories independently, not as a hierarchy. For example:
...
How you tag container images depends on your scenarios for developing or deploying them. For example, use stable tags to maintain your base images, and use unique tags for deploying images. A clear tagging strategy is essential for versioning and rollout management.

For tag naming rules, follow the general Docker tagging conventions and keep names consistent, descriptive, and easy to trace.

### Layer

Container images and artifacts consist of one or more _layers_. Different artifact types define layers differently. For example, in a Docker container image, each layer corresponds to a line in the Dockerfile that defines the image:

![[container-image-layers.png]]
...
Each container image or artifact you push to a container registry has a _manifest_. The registry generates the manifest when you push the content. The manifest uniquely identifies the artifacts and specifies the layers.
...
Use the Azure CLI command az acr manifest list-metadata to list the manifests for a repository:
...
Manifests have a unique SHA-256 hash called a _manifest digest_. Each image or artifact - whether tagged or not - has its own digest. The digest value is unique even if the artifact's layer data matches another artifact. This feature lets you push identically tagged images to a registry without any problems. For example, you can push `myimage:latest` to your registry over and over without an error, because each image has its own unique digest.
...
> **Important:** If you repeatedly push modified artifacts with identical tags, you might create **orphans**: artifacts that are untagged but still use space in your registry. Untagged images don't show up in the Azure CLI or in the Azure portal when you list or view images by tag. However, their layers still exist and use space in your registry. Deleting an untagged image frees registry space when the manifest is the only one or the last one pointing to a particular layer. In practice, regular cleanup is important to avoid unnecessary storage growth.
